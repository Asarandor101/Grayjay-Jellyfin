/*
 * Jellyfin -> Grayjay experimental source, v4
 * Authentication is handled by Grayjay. The plugin never receives the Jellyfin token directly.
 */
...
source.getContentDetails = function(url) {
    ...
    for (let i = 0; i < mediaSources.length; i++) {
        ...
        videoSources.push(new VideoUrlSource({
            ...
            url: SERVER + "/Videos/" + enc(item.Id) + "/stream?Static=true&MediaSourceId=" + enc(ms.Id || "")
        }));
    }
    ...
    return new PlatformVideoDetails({
        ...
        author: new PlatformAuthorLink(item.SeriesName || "Jellyfin", SERVER, SERVER, ""),
...
function ensureUser() {
    if (!USER) USER = apiJson("/Users/Me");
}

function apiJson(path) {
    const headers = {
        "Accept": "application/json",
        "Authorization": 'MediaBrowser Client="Grayjay Jellyfin", Device="Android", DeviceId="grayjay-jellyfin", Version="0.4"'
    };
    const res = http.GET(SERVER + path, headers, true);
    if (!res || !res.isOk)
        throw new ScriptException(
            "Jellyfin request failed: " + path +
            " (HTTP " + (res ? res.code : "?") + "). Open the source and sign in to Jellyfin first."
        );
